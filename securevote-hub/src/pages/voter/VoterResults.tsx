...
export default function VoterResults() {
  ...
  useEffect(() => {
    contractService.getElections().then((els) => {
      setElections(els);
      const ended = els.filter((e) => e.status === "ended" || e.status === "active");
      if (ended.length > 0) setSelectedElection(ended[0].id);
    });
  }, []);

  const currentElection = elections.find((e) => e.id === selectedElection);
  const chartData = currentElection?.candidates.map((c) => ({ name: c.name, votes: c.votes })) || [];
  ...
  return (
    <DashboardLayout role="voter">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">View election results from the blockchain</p>
      </div>
      ...
      <div className="mb-6">
        <Select value={selectedElection} onValueChange={setSelectedElection}>
          ...
          <SelectContent>
            {elections.filter((e) => e.status !== "upcoming").map((e) => (<SelectItem key={e.id} value={e.id}>{e.title}</SelectItem>))}
          </SelectContent>
        </Select>
      </div>

      {winner && winner.votes > 0 && currentElection?.status === "ended" && (
        ...
        {chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ backgroundColor: "hsl(230, 25%, 12%)", border: "1px solid hsl(230, 20%, 22%)", borderRadius: "8px" }} />
          ...
          <p className="py-12 text-center text-muted-foreground">No results available</p>
        )}
      </div>
    </DashboardLayout>
  );
